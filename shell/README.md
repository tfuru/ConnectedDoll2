...
2. **ボトムケース**: `scad/bottom_case.scad`
   - 前面大型ボタン開口部、右側面ボリュームスリット、スピーカー固定ボス（M1.8）、電池ボックス収容部（**三角バットレス補強ガイドリブ＆側壁一体ストッパー**）、底面ネジザグリ穴＆基板支柱ボス、**前面ボタン用コイルスプリング受け座ボス（左右2箇所）**、**前面スピーカー出音用スリット（幅1.2mm×高さ6.0mm×7連バーチカルスリット、スピーカー出音口に対向）**
3. **前面ボタン（ボタンキャップ）**: `scad/front_button.scad`
   ...
   - **M2ネジ調整式スイッチ押下機構**:
     - ボタン裏面に M2 下穴ボス（外径φ4.4mm / 下穴φ1.7mm / 突出高0.6mm）を配置。基板上面との干渉を避けるDカット逃げ加工済み。
