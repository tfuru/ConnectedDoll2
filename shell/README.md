...
- **ボトムケース**:
  ...
  - **右側面**: 音量調整用 スリット開口部（約 16mm × 4.5mm, R=1.5mm, 前方5mmオフセット / アナログ可変抵抗 RK10J11R0A0H 操作用）
  - **内部底面**:
...
- **一体成型バネ構造（ハイブリッド弾性機構 & 内部スイッチポケット方式）**:
  - **裏面スイッチ収容ポケット（干渉完全ゼロ）**: 基板Top層前端から突出するタクトスイッチ（EVQPUC02K）をボタン裏面のリセス（深さ1.4mm）内に完全に収容し、本体へのめり込み干渉を防止。
  - **内蔵カンチレバー弾性押下面**: ポケット内部に配置された弾性押圧部が、非押下時にスイッチ先端と **0.2mm の適正クリアランス** を保ち、押下時にスイッチの微小ストローク（0.25mm）を超えて約 **1.2mm** の心地よい深みのある押し込みストロークとソフトな底付き感を実現。
