...
### 2. 前面大型ボタン機構（側面配置・コイルスプリング＆M2アジャスタブル・プランジャー）
- ボトムケースの正面側壁に大型ボタンパーツを配置。
- **M2ネジ式アジャスタブル・プランジャー機構（基板タクトスイッチ整合）**:
  - **精密高さ整合**: 20mm支柱スペーサー上の基板表面に実装されたタクトスイッチ（EVQPUC02K: 中心高さ Z=24.625mm / ボタンオフセット Y=+1.625mm）のアクチュエータ位置に完全一致するよう M2 ネジボスを配置。
  - **基板前端面衝突防止（ボス低背化＆Dカット逃げ）**: ボス突出高さを 0.6mm に抑え、さらに基板上面（Z=23.8mm）より下側を逃げる Dカット形状を採用することで、ボタン押下時でも基板エッジとの衝突が一切生じない設計。
  - **M2ネジによる無段階クリアランス調整**: M2 なべ小ねじ（L=4〜6mm）をねじ込み、スイッチ先端とのクリアランス（0.2mm〜0.4mm）および押し込み深さをドライバーで自由に微調整可能。
...
- がたつき防止ガイドおよび適度な押下ストローク（約0.3〜0.5mm）を確保。
